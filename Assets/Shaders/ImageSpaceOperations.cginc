...
inline float3 findViewPos(Texture2D<float> zbuffer, uint2 id, float2 uv, float4x4 invProjMat)
{
    const float z = 1.0f - 2 * zbuffer[id].r;
    const float x = uv.x * 2 - 1;
    const float y = uv.y * 2 - 1;
    float4 viewPos = mul(invProjMat, float4(x, y, z, 1.0f));
    return viewPos.xyz / viewPos.w;
...
inline float3 findViewNormal(Texture2D<float4> nbuffer, uint2 id, float4x4 viewMat)
{
